...
fn parse_array_index(pair: Pair<Rule>) -> ArrayIndex {
    ...
    let base = Box::new(match name.as_rule() {
        ...
        Rule::string => Base::StringLiteral(rho_lexer::parse_string(name.as_str())),
    ...
    PRATT_PARSER
        .map_primary(|primary| match primary.as_rule() {
            ...
            Rule::string => Expr::Primary(Primary::Literal(Literal::String(
                rho_lexer::parse_string(primary.as_str()), //parse string
            ))),
            ...
            Rule::null => Expr::Primary(Primary::Literal(Literal::Null)),
            Rule::char => Expr::Primary(Primary::Literal(Literal::Char(
                rho_lexer::parse_string(primary.as_str())
                    .chars()
                    .next()
                    .unwrap(), //parse string
            ))),
            Rule::record_field_rv => {
                Expr::Primary(Primary::RecordField(parse_record_field(primary)))
            }
...
fn type_to_enum(pair: Pair<Rule>) -> VarType {
    ...
    match t.as_rule() {
        ...
        Rule::identifier => VarType::Identifier(t.as_str().to_string()),
        _ => panic!("Expected string \"int\" or \"bool\""),
    }
}
...
fn parse_array(pair: Pair<Rule>) -> VarType {
    ...
    let arr_type = match inner.next().unwrap().as_str() {
        "int" => VarType::IntType,
        "bool" => VarType::BoolType,
        s => VarType::Identifier(s.to_string()),
    };
...
///precondition: input is an arbitrary number of identifiers and one type
fn mult_decl_to_type_pair(pair: Pair<Rule>) -> Vec<(String, VarType)> {
    let mut inner = pair.into_inner();
    let mut idents = vec![];
    let mut next = inner.next().unwrap();
    while let Rule::identifier = next.as_rule() {
        let id = String::from(next.as_str());
        idents.push(id);
        next = inner.next().unwrap();
    }
    let t = type_to_enum(next);
    idents.into_iter().map(|x| (x, t.clone())).collect()
}
...
fn parse_record_field(pair: Pair<Rule>) -> RecordField {
    let line_col = pair.line_col();
    let mut inner = pair.into_inner();
    let name = inner.next().unwrap();
    let base = Box::new(match name.as_rule() {
        ...
        Rule::method_call => Base::FunctionCall(parse_function_call(name)),
        Rule::expr => Base::Expr(parse_expr(name.into_inner())),
        Rule::array_lv => Base::ArrayIndex(parse_array_index(name)),
        _ => unreachable!("Record field base should not be {:?}", name),
    });
    ...
    inner
        ...
        .unwrap()
        .into_inner()
        .fold(RecordField::Base(base), |acc, x| {
            RecordField::Field(
                Box::new(acc),
                Box::new(parse_expr(x.into_inner().next().unwrap().into_inner())),
                line_col.0,
...
fn parse_lvalues(pair: Pair<Rule>) -> Vec<LValue> {
    ...
    pair.into_inner()
        .map(|lvalue| match lvalue.as_rule() {
            ...
            Rule::record_field_lv => LValue::RecordField(parse_record_field(lvalue), line_col),
...
fn parse_record(pair: Pair<Rule>) -> Assignment {
    let line_col = pair.line_col();
    let mut inner = pair.into_inner();
    let id = inner.next().unwrap();
    let mut fields = vec![];
    while let Some(i) = inner.next() {
        fields.extend(mult_decl_to_type_pair(i.clone()));
    }
    let f: HashMap<String, VarType> = fields.clone().into_iter().collect();
    Assignment {
        lvalues: vec![LValue::TypePair(
            (
                id.as_str().to_string(),
...
fn parse_glob_decl(pair: Pair<Rule>) -> Assignment {
    ...
    if let Rule::record_decl = pair.clone().into_inner().next().unwrap().as_rule() {
        parse_record(pair.into_inner().next().unwrap())
    } else {
        let mut inner = pair.into_inner();
        ...
        match rvalue {
            ...
            None => Assignment {
                lvalues: mult_decl_to_type_pair(lvalue)
                    .into_iter()
                    .map(|x| LValue::TypePair(x, line_col))
                    .collect(),
                rvalues: None,
                line_col,
            },
...
fn parse_assignment(pair: Pair<Rule>) -> Assignment {
    ...
    match inner.next() {
        ...
        None => Assignment {
            lvalues: mult_decl_to_type_pair(lvalues.into_inner().next().unwrap())
                .into_iter()
                .map(|x| LValue::TypePair(x, line_col))
                .collect(),
            rvalues: None,
...
fn parse_statement(pair: Pair<Rule>) -> Statement {
    ...
    match pair.as_rule() {
        ...
        Rule::loop_stmt => match inner.as_rule() {
            Rule::loop_block => Statement::Block(parse_block(inner)),
            Rule::loop_if_stmt => Statement::If(parse_if(inner)),
            Rule::block => Statement::Block(parse_block(inner)),
            Rule::while_block => Statement::While(parse_while(inner)),
            Rule::if_stmt => Statement::If(parse_if(inner)),
            Rule::assignment => Statement::Assignment(parse_assignment(inner)),
            ...
            Rule::method_call => Statement::ProcedureCall(parse_procedure_call(inner)),
            Rule::loop_break => Statement::Break(Break { line_col }),
            _ => unreachable!("Failed parsing command"),
        },
...
/// Function to parse a function or procedure declaration in .ri files
...
pub fn parse(contents: &str) -> Result<File, Error<Rule>> {
    let pest_program = RhoParser::parse(Rule::program, contents)?.next().unwrap();
    ...
    let decls: Vec<Declaration> = program
        .next()
        .unwrap()
        .into_inner()
...
pub fn parse_interface(contents: &str) -> Result<File, Error<Rule>> {
    let pest_program = RhoParser::parse(Rule::interface, contents)?.next().unwrap();
    let line_col = pest_program.line_col();
    let mut program = pest_program.into_inner();
    let uses = program
        .next()
    ...
    let decls: Vec<Declaration> = program
        ...
        .into_inner()
        .map(|x| match x.as_rule() {
            Rule::interface_function => Declaration::InterfaceFn(parse_interface_fn(x)),
            Rule::record_decl => Declaration::GlobalDecl(parse_record(x)),
            _ => unreachable!("This case should never occur."),
        })
        .collect();
